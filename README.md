...
- 调用 `SkTokenService.validSkToken(date, trainCode)`
  ...
  （1） 通过车次，日期，会员ID获取分布式锁，避免机器人刷票

​	（2）这里在Redis做了缓存，先去Redis扣减令牌库存，当令牌对5求余=0时，再去做数据库的同步
...
- 对每节车厢内的座位按序号遍历
- 判断条件:
  1. 座位未被预选（主要是为了防止无选座模式下，多人选座重复(多人选座位一个一个选)，定义了finalSeatList，记录了已经完成选票的人，在这里对比座位号）
  2. 列号符合要求（选座模式）
